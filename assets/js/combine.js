...
let cssCombined = cssFiles
  .map((file) => fs.readFileSync(file, "utf8"))
  .join("\n");

// Combine JS
const jsFiles = ["assets/js/app.js"];
let jsCombined = jsFiles
  .map((file) => fs.readFileSync(file, "utf8"))
  .join("\n");
...
if (headCloseIdx !== -1) {
  const assetsBlock = `\n<!-- Combined Assets -->\n<style>\n${cssCombined}\n</style>\n<script>\n${jsCombined}\n</script>\n`;
  indexHtml =
    indexHtml.slice(0, headCloseIdx) +
    assetsBlock +
...
function extractBody(html) {
  ...
  body = body.replace(
    /<script[^>]*src="assets\/js\/app\.js"[^>]*><\/script>/gi,
    "",
  );
  return body;
}
...
for (const file of files) {
  // Remove any existing block for this file using start/end comments
  const blockPattern = new RegExp(
    `<!-- Start ${file} -->[\\s\\S]*?<!-- End ${file} -->\\n?`,
    "g",
  );
  indexHtml = indexHtml.replace(blockPattern, "");
...
if (bodyCloseIdx !== -1) {
  indexHtml =
    indexHtml.slice(0, bodyCloseIdx) +
    combinedSections +
    indexHtml.slice(bodyCloseIdx);
}
